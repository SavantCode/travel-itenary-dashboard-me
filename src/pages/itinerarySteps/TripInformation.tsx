//v3 - Final Version with robust saving logic final fianl
...
export default function App() {
  ...
  // **IMPROVEMENT**: Load data from the correct top-level 'tripInformation' field.
  useEffect(() => {
    try {
      ...
      if (savedItineraryString) {
        ...
        let tripInfo = savedItinerary.tripInformation;

        // Fallback for old data structure for backward compatibility.
        if (!tripInfo && Array.isArray(savedItinerary) && savedItinerary[0]?.['Trip Information']) {
          tripInfo = savedItinerary[0]['Trip Information'];
        }

        if (Array.isArray(tripInfo) && tripInfo.length > 0) {
          setSections(tripInfo);
        }
      }
    } catch (error) {
  ...
  const handleSubmit = () => {
      try {
          ...
          // **IMPROVEMENT**: Save trip information to a separate, top-level field.
          const updatedItinerary = {
              ...currentItinerary,
              tripInformation: sections
          };

          sessionStorage.setItem('editedItinerary', JSON.stringify(updatedItinerary));

          setSaveStatus('success');
          console.log("Updated Itinerary with Trip Information:", updatedItinerary);

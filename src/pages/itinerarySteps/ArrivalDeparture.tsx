...
const InputField: FC<InputFieldProps> = ({ label, name, value, onChange, placeholder, type = 'text', options, containerClassName = '', labelClassName = '' }) => {
  ...
  const renderInput = () => {
    switch (type) {
      case 'select':
        const dataListId = `datalist-${name}-${Math.random().toString(36).substr(2, 9)}`;
        return (
          ...
          {legs.map((leg, index) => (
            <div key={leg.id} className="border rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                ...
                <SummaryRow label="To" value={leg.toCity} />
...
const ArrivalDeparturePage: FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (savedItineraryString) {
        ...
        // **IMPROVEMENT**: Load data from the correct top-level 'arrivalDeparture' field for robustness.
        if (savedItinerary && Array.isArray(savedItinerary.arrivalDeparture) && savedItinerary.arrivalDeparture.length > 0) {
          setJourneyLegs(savedItinerary.arrivalDeparture);
          return;
        }
        // Fallback for old data structure for backward compatibility
        if (Array.isArray(savedItinerary) && savedItinerary[0]?.ArrivalandDeparture) {
            setJourneyLegs(savedItinerary[0].ArrivalandDeparture);
            return;
        }
      }
    } catch (error) {
    ...
    if (journeyLegs.length === 0) {
        setJourneyLegs([{
          ...
          airline: 'Emirates', flightNo: '', airport: '',
        }]);
    }
  ...
  const handleLegChange = (id: number, e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setJourneyLegs(updatedLegs);
  };

  const addLeg = () => {
    setJourneyLegs([ ...journeyLegs, {
        id: Date.now(),
        modeOfTravel: 'Flight', fromCity: '', toCity: '', date: '', time: '',
        airline: '', flightNo: '', airport: '',
      },
    ]);
  };
  ...
  const removeLeg = (id: number) => {
    if (journeyLegs.length > 1) {
        ...
        alert("You cannot remove the last journey leg.");
    }
  };
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    try {
        ...
        // **IMPROVEMENT**: Save arrival/departure details to a separate, top-level field.
        // This mirrors the robust way 'banner' data is saved and prepares it for MongoDB.
        const updatedItinerary = {
            ...currentItinerary,
            arrivalDeparture: journeyLegs
        };

        sessionStorage.setItem('editedItinerary', JSON.stringify(updatedItinerary));

        alert("Saved Arrival & Departure Details!");
        console.log("Updated Itinerary with Arrival/Departure:", updatedItinerary);
        navigate('/my-itinerary/create/accommodation');
    } catch (error) {
        console.error("Failed to save data to sessionStorage:", error);
        alert("Could not save the data. An error occurred.");
    }
  };
      ...
      <div className="bg-[#F6F6FA] min-h-screen p-4 sm:p-6 lg:p-2 font-raleway">
        <header className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center space-x-4">
            <button type="button" onClick={addLeg} className="flex items-center space-x-2 text-xs text-[#727171] cursor-pointer hover:text-[#10A4B0]">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          <div className="lg:col-span-2 bg-white p-4 rounded-lg border border-[#E0E0E0] space-y-6">
            <img src="https://images.unsplash.com/photo-1523833082115-1e8e294bd14e?q=80&w=1295&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG9otby1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" alt="Airplane on runway" className="w-full h-44 object-cover rounded-md shadow-md" />
            {journeyLegs.map((leg, index) => (
              <div key={leg.id} className="p-4 border border-gray-200 rounded-lg space-y-4 relative pt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <InputField label="From" name="fromCity" value={leg.fromCity} onChange={(e) => handleLegChange(leg.id, e)} type="select" options={['Dubai (UAE)', 'Singapore (SG)', 'New York (USA)', 'London (UK)']} />
                  <InputField label="To" name="toCity" value={leg.toCity} onChange={(e) => handleLegChange(leg.id, e)} type="select" options={['Dubai (UAE)', 'Singapore (SG)', 'New York (USA)', 'London (UK)']} />
            ...
            <div className="bg-white p-4 rounded-lg border border-[#E0E0E0]">
              ...
              <div className="border border-[#E0E0E0] rounded-lg">
                ...
                <div className="p-4 space-y-3">
                  ...
                  <SummaryRow label="To" value={journeyLegs[0]?.toCity} />
...
// //v3 saving data - with robust "Check, Create, and Update" logic
// import React, { useState, ChangeEvent, FC, FormEvent, useEffect } from 'react';
// import { useNavigate } from 'react-router-dom';
// import { Eye, Plus, Calendar, ChevronDown, MoreHorizontal, Trash2, X, Plane, Train, Bus, Car } from 'lucide-react';
...
//         if (dayData && Array.isArray(dayData.ArrivalandDeparture) && dayData.ArrivalandDeparture.length > 0) {
//           setJourneyLegs(dayData.ArrivalandDeparture);
//           return; 
//         }
//       }
//     } catch (error) {
...
//     // FALLBACK: If no data was loaded from storage, initialize with a default leg.
//     if (journeyLegs.length === 0) {
//         setJourneyLegs([{
//           id: Date.now(),
//           modeOfTravel: 'Flight',
...
//             } catch (err) {
//                 console.error("Error parsing existing itinerary, starting fresh.", err);
//                 fullItinerary = [{}];
//             }
//         } else {
//             fullItinerary = [{}];
...
//             ...fullItinerary[0],
//             "ArrivalandDeparture": journeyLegs
//         };
...
// import React, { useState, ChangeEvent, FC, FormEvent, useEffect } from 'react';
// import { useNavigate } from 'react-router-dom';
// // import { useItinerary } from '../../context/ItineraryContext'; // FIX: Removed this import as the context is not available.
// import { Eye, Plus, Calendar, ChevronDown, MoreHorizontal, Trash2, X, Plane, Train, Bus, Car } from 'lucide-react';
...
//   modeOfTravel: 'Flight' | 'Train' | 'Bus' | 'Car';
//   fromCity: string;
//   toCity: string; // ADDED: "To" destination field
//   date: string;
//   time: string;
//   airline?: string;
//   flightNo?: string;
...
// // SECTION: Reusable Sub-Components
// // *** MODIFIED *** InputField to allow manual entry in dropdowns
// const InputField: FC<InputFieldProps> = ({ label, name, value, onChange, placeholder, type = 'text', options, containerClassName = '', labelClassName = '' }) => {
//   const commonInputClasses = "w-full bg-gray-100 border border-gray-200 rounded-[5px] h-[30px] px-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#10A4B0]";
//   const renderInput = () => {
//     switch (type) {
...
// // *** MODIFIED *** PreviewModal to include "To" city
// const PreviewModal: FC<PreviewModalProps> = ({ legs, onClose }) => {
//   const travelModeIcons = {
//     Flight: <Plane className="w-5 h-5 text-[#10A4B0]" />,
//     Train: <Train className="w-5 h-5 text-[#10A4B0]" />,
...
// // SECTION: Main Page Component
// const ArrivalDeparturePage: FC = () => {
//     // FIX: Replaced context with local state management.
//     const [journeyLegs, setJourneyLegs] = useState<JourneyLeg[]>([]);
//     // This is a placeholder since the original `useNavigate` won't work in this environment.
//     const navigate = (path: string) => console.log(`Navigating to: ${path}`);
//     const [isPreviewOpen, setIsPreviewOpen] = useState(false);

//   // *** MODIFIED LOGIC ***
//   // On component mount, this tries to load existing data from sessionStorage.
//   // If it fails or finds no data, it initializes the form with a default leg.
//   useEffect(() => {
//     try {
//       const savedItineraryString = sessionStorage.getItem('editedItinerary');
//       if (savedItineraryString) {
//         const savedItinerary = JSON.parse(savedItineraryString);
//         const dayData = savedItinerary[0]; // Assuming we're editing the first day's itinerary

//         // Check if ArrivalandDeparture data exists and is a valid array
//         if (dayData && Array.isArray(dayData.ArrivalandDeparture) && dayData.ArrivalandDeparture.length > 0) {
//           setJourneyLegs(dayData.ArrivalandDeparture);
//           return; // Exit early as we've successfully loaded the data
//         }
//       }
//     } catch (error) {
//       console.error("Failed to load or parse 'editedItinerary' from sessionStorage:", error);
...
//   // *** MODIFIED LOGIC (from previous step) ***
//   // This function now merges the form data into the main itinerary object in sessionStorage.
//   const handleSubmit = (e: FormEvent) => {
//     e.preventDefault();
//     try {
...
//       const savedItineraryString = sessionStorage.getItem('editedItinerary');
//       if (!savedItineraryString) {
//         alert("CRITICAL ERROR: Main itinerary data not found. Cannot save.");
//         return;
//       }
//       const fullItinerary = JSON.parse(savedItineraryString);

//       // 2. Ensure it's in the expected array format
//       if (!Array.isArray(fullItinerary) || fullItinerary.length === 0) {
//         alert("CRITICAL ERROR: Invalid itinerary format found.");
//         return;
//       }

//       // 3. Create an updated version of the first day's data by merging the new field
//       const updatedDayData = {
//         ...fullItinerary[0], // Copy all existing data (title, sightseeing, Trip Information, etc.)
//         "ArrivalandDeparture": journeyLegs // Add or overwrite the ArrivalandDeparture field with form data
//       };

//       // 4. Construct the new full itinerary array with the updated data
//       const updatedItinerary = [updatedDayData, ...fullItinerary.slice(1)];
      
//       // 5. Save the complete, updated itinerary back to session storage
//       sessionStorage.setItem('editedItinerary', JSON.stringify(updatedItinerary));

//       alert("Saved Arrival & Departure Details!"); // Provide user feedback
//       console.log("Updated Itinerary with Arrival/Departure:", updatedItinerary); // For debugging

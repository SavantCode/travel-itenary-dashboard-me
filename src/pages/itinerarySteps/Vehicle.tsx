...
// ==========================================================================================Data saving code to editedItinerary==============================================================================================================================================================================================================================================================================
...
//   // *** MODIFIED LOGIC ***
//   // Load existing vehicle data from sessionStorage when component mounts
//   useEffect(() => {
//     try {
...
//             if (dayData && Array.isArray(dayData.selectedVehicles)) {
//                 setSavedVehicles(dayData.selectedVehicles);
//             }
//         }
//     } catch (error) {
//         console.error("Failed to load vehicle data from sessionStorage:", error);
//     }
//   }, []);
...
//   // *** MODIFIED LOGIC ***
//   // This function saves all vehicle bookings to the 'editedItinerary' object in sessionStorage.
//   const handleFinalSubmit = () => {
//     if (savedVehicles.length === 0) {
//         alert("No bookings to submit. Please add at least one vehicle booking.");
//         return;
...
//             ...fullItinerary[0],
//             "selectedVehicles": savedVehicles // Add/overwrite the selectedVehicles field
//         };
...
//         alert(`${savedVehicles.length} vehicle booking(s) have been submitted and saved to the main itinerary.`);
//         console.log("Updated Itinerary with Vehicles:", updatedItinerary);
//     } catch (error) {
...
//                     value={type}
//                     checked={currentVehicle.driveType === type}
//                     onChange={() => handleInputChange('driveType', type as DriveType)}
//                     className="h-3.5 w-3.5 accent-[#10A4B0]"
//                   />
//                   {type === 'self-drive' ? 'Self Drive' : 'Vehicle with Driver'}
//                 </label>
...
//             <div className=" rounded-md border border-[#E0E0E0]">
//                 <div className="bg-[#10A4B0] text-white flex justify-between items-center px-3 py-3 rounded-t-md">
//                     <h3 className="font-raleway font-medium text-lg">Bookings List</h3>
//                 </div>
...
//         {successMessage && (
//             <div className="mt-4 p-3 bg-green-100 border border-green-300 text-green-800 text-xs rounded-md animate-fade-in-out shadow-sm">
//             {successMessage}
//             </div>
//         )}
//         </div>
...
//                 <h3 className="font-bold text-gray-800 text-sm">{vehicle.vehicleType}</h3>
//                 <div className="flex items-center gap-2">
//                     <button type="button" onClick={onEdit} className="p-1 text-gray-500 hover:text-[#10A4B0] rounded-full hover:bg-gray-200 transition-colors"><EditIcon /></button>
//                     <button type="button" onClick={onDelete} className="p-1 text-gray-500 hover:text-red-500 rounded-full hover:bg-red-100 transition-colors"><TrashIcon /></button>
//                 </div>
//             </div>
//             <div className="p-3 space-y-3">
//                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
...
//         const updatedDayData = {
//             ...fullItinerary[0],
//             "selectedVehicles": savedVehicles
//         };
...
//               <h2 className="font-semibold text-base text-gray-800">{isEditing ? `Editing Booking for: ${currentVehicle.vehicleType}` : 'Add New Vehicle Booking'}</h2>
//               {savedVehicles.length > 0 && (
//                   <button type="button" onClick={handleAddNew} className="text-xs font-medium text-[#10A4B0] hover:underline disabled:text-gray-400 disabled:no-underline" disabled={!isEditing}>
//                       + Add New Booking
//                   </button>
//               )}
//           </div>
...
// ========================================================================================================================
//fianl robust data saving ..that's it
...
const VehicleBooking: React.FC = () => {
  ...
  // **IMPROVEMENT**: Load data from the correct top-level 'selectedVehicles' field.
  useEffect(() => {
    try {
        const savedItineraryString = sessionStorage.getItem('editedItinerary');
        if (savedItineraryString) {
            ...
            if (savedItinerary && Array.isArray(savedItinerary.selectedVehicles)) {
                setSavedVehicles(savedItinerary.selectedVehicles);
                return; // Exit after successful load
            }
            
            // Fallback for old data structure for backward compatibility.
            if (Array.isArray(savedItinerary) && savedItinerary[0]?.selectedVehicles) {
                setSavedVehicles(savedItinerary[0].selectedVehicles);
                return; // Exit after successful load
            }
        }
    } catch (error) {
    ...
    try {
        ...
        if (savedItineraryString) {
            ...
            if (Array.isArray(parsedData)) {
                currentItinerary = { days: parsedData }; // Convert old array format to new object
            } else {
                currentItinerary = parsedData;
            }
        }
        
        // **IMPROVEMENT**: Save vehicle details to a separate, top-level field.

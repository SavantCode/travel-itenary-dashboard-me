//v4
import React, { useState, useRef, ChangeEvent, useCallback, memo, FC } from 'react';
import { Eye, Image as ImageIcon, Trash2, Plus, X, BusFront, Maximize, Minimize, Car, Utensils, MapPin } from 'lucide-react';
...
const initialDay: Day = {
  id: Date.now(),
  date: new Date('2025-08-27T00:00:00'),
...
const MemoizedDayPreviewCard: FC<{ day: Day; onDelete: (id: number) => void; onImageLayoutToggle: (id: number) => void; isDeleteDisabled: boolean }> = memo(({ day, onDelete, onImageLayoutToggle, isDeleteDisabled }) => (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
      <div className="bg-[#10A4B0] text-white p-3 rounded-t-lg flex justify-between items-center">
        ...
        <div className="flex space-x-1.5">{[...Array(3)].map((_, i) => <span key={i} className="w-3 h-3 bg-white/70 rounded-full"></span>)}</div>
      </div>
      <div className="p-4">
      ...
      <div className="p-4 flex gap-2">
        ...
        <button onClick={() => onDelete(day.id)} disabled={isDeleteDisabled} className="p-2 bg-red-50 text-red-500 rounded-md hover:bg-red-100 disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed">
...
const DayItineraryForm: FC<{ day: Day; onUpdate: (field: keyof Day, value: any) => void; onImageUpload: (e: ChangeEvent<HTMLInputElement>) => void; fileInputRef: React.RefObject<HTMLInputElement> }> = memo(({ day, onUpdate, onImageUpload, fileInputRef }) => (
    <div className="space-y-6 animate-fade-in">
        <div>
            <label className="text-xs font-medium text-gray-500">Main Title for the Day</label>
            <input type="text" placeholder="e.g., Arrival & City Exploration" value={day.title} onChange={(e) => onUpdate('title', e.target.value)} className="w-full mt-1 p-2 border border-gray-300 rounded-md text-sm focus:ring-1 focus:ring-[#10A4B0] focus:border-[#10A4B0] transition" />
        </div>
        ...
        <div>
            ...
            <ReactQuill value={day.remark} onChange={(value) => onUpdate('remark', value)} modules={{ toolbar: [['bold', 'italic', 'link']] }} theme="snow" className="quill-editor" />
        </div>
    </div>
));

const SightseeingForm: FC<{ day: Day; onUpdate: (field: keyof Day, value: any) => void }> = memo(({ day, onUpdate }) => {
    const fileInputRefs = useRef<{ [key: number]: HTMLInputElement | null }>({});
    const handleItemChange = (itemId: number, field: keyof SightseeingItem, value: any) => {
        onUpdate('sightseeing', day.sightseeing.map((item: SightseeingItem) => item.id === itemId ? { ...item, [field]: value } : item));
    };
    const handleImageForItem = (e: ChangeEvent<HTMLInputElement>, itemId: number) => {
        if (e.target.files && e.target.files[0]) {
            handleItemChange(itemId, 'image', URL.createObjectURL(e.target.files[0]));
        }
    };
    const addItem = () => {
        onUpdate('sightseeing', [...day.sightseeing, { id: Date.now(), name: '', time: '', image: DEFAULT_IMAGES.sightseeing, imageLayout: 'cover', description: '', ticketsIncluded: false }]);
    };
        ...
        <div className="space-y-6 animate-fade-in">{day.sightseeing.map((item: SightseeingItem, index: number) => (
            <div key={item.id} className="bg-gray-50 p-4 rounded-lg border relative">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="flex-shrink-0 w-full md:w-40 h-28 bg-gray-200 rounded-md relative cursor-pointer group" onClick={() => fileInputRefs.current[item.id]?.click()}>
                        <img src={item.image} alt={item.name} className="w-full h-full object-cover rounded-md" />
                        <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 flex items-center justify-center transition-all"><ImageIcon className="w-6 h-6 text-white opacity-0 group-hover:opacity-100" /></div>
                    </div>
                    <input type="file" accept="image/*" ref={el => fileInputRefs.current[item.id] = el} onChange={(e) => handleImageForItem(e, item.id)} className="hidden" />
...
const TransfersForm: FC<{ day: Day; onUpdate: (field: keyof Day, value: any) => void }> = memo(({ day, onUpdate }) => {
    const handleItemChange = (itemId: number, field: keyof TransferItem, value: any) => {
        onUpdate('transfers', day.transfers.map(item => item.id === itemId ? { ...item, [field]: value } : item));
    };
...
const MealsForm: FC<{ day: Day; onUpdate: (field: keyof Day, value: any) => void }> = memo(({ day, onUpdate }) => {
    const fileInputRefs = useRef<{ [key: number]: HTMLInputElement | null }>({});
    ...
    const handleImageForItem = (e: ChangeEvent<HTMLInputElement>, itemId: number) => { if (e.target.files && e.target.files[0]) { handleItemChange(itemId, 'image', URL.createObjectURL(e.target.files[0])); } };
            ...
            <div key={item.id} className="bg-gray-50 p-4 rounded-lg border relative">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <div className="flex-grow grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <select value={item.type} onChange={e => handleItemChange(item.id, 'type', e.target.value)} className="p-2 border rounded-md text-sm w-full bg-white h-fit">
                            <option>Breakfast</option><option>Lunch</option><option>Dinner</option><option>Snack</option>
                        </select>
                        <input type="text" placeholder="Restaurant / Location" value={item.location} onChange={e => handleItemChange(item.id, 'location', e.target.value)} className="p-2 border rounded-md text-sm w-full h-fit" />
                        <textarea placeholder="Inclusions (e.g., Set menu)" value={item.inclusions} onChange={e => handleItemChange(item.id, 'inclusions', e.target.value)} className="sm:col-span-2 p-2 border rounded-md text-sm w-full resize-none h-14"></textarea>
                    </div>
                </div>
                <div className="flex items-center gap-2 mt-3">
...
const DayWiseItineraryPage: React.FC = () => {
  ...
  const handleImageUpload = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0] && activeDay) {
      handleUpdateDay(activeDay.id, 'image', URL.createObjectURL(e.target.files[0]));
    }
  ...
  const renderActiveForm = () => {
      ...
      switch(activeDay.activeTab) {
          case 'Day Itinerary': return <DayItineraryForm day={activeDay} onUpdate={(field: keyof Day, value: any) => handleUpdateDay(activeDay.id, field, value)} onImageUpload={handleImageUpload} fileInputRef={fileInputRef} />;
          case 'Sightseeing': return <SightseeingForm day={activeDay} onUpdate={(field: keyof Day, value: any) => handleUpdateDay(activeDay.id, field, value)} />;
          case 'Transfers': return <TransfersForm day={activeDay} onUpdate={(field: keyof Day, value: any) => handleUpdateDay(activeDay.id, field, value)} />;
          case 'Meals': return <MealsForm day={activeDay} onUpdate={(field: keyof Day, value: any) => handleUpdateDay(activeDay.id, field, value)} />;
          default: return null;
      }
  };
  ...
  return (
    <div className="bg-[#F6F6FA] min-h-screen font-sans text-[#1E1E1E]">
      ...
      <div className="test w-full mx-auto px-1 sm:px-6 lg:px-2 py-2">
        <header className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center space-x-8">
            ...
            <button onClick={() => setIsPreviewOpen(true)} className="flex items-center space-x-2 bg-[#10A4B0] text-white px-3 py-1 rounded-full text-xs font-inter font-medium">
        ...
        {/* IMPROVEMENT 2: Main layout now uses Flexbox. This allows re-ordering of columns on different screen sizes. */}
        <main className="flex flex-col xl:flex-row gap-8 items-start">
          ...
          {/* IMPROVEMENT 4: The preview is order-1 (first on mobile) and order-2 (second on desktop), and is only sticky on desktop. */}
      ...
      {/* --- PREVIEW MODAL (Remains unchanged, already responsive) --- */}
      {isPreviewOpen && (
              ...
              {days.map((day, index) => (
                <div key={day.id} className="mb-8 last:mb-0">
                  <h3 className="text-xl font-semibold text-gray-800 mb-3 pb-2 border-b-2 border-[#10A4B0]">
                    Day {index + 1}: {day.title} ({day.date.toLocaleString('en-US', { month: 'short', day: 'numeric' })})
                  </h3>

...
import { Eye, Image as ImageIcon, Trash2, Plus, X, BusFront, Maximize, Minimize, Car, Utensils, MapPin, Loader2, Megaphone } from 'lucide-react';
...
const DEFAULT_IMAGES = {
  tripBanner: 'https://images.unsplash.com/photo-1501785888041-af3ef285b470?q=80&w=2070&auto=format&fit=crop',
...
interface Day { id: number; date: string; title: string; description: string; image: string; imageLayout: ImageLayout; remark: string; sightseeing: SightseeingItem[]; transfers: TransferItem[]; meals: MealItem[]; activeTab: ActivityTab; }
interface BannerDetails { title: string; description: string; image: string; imageLayout: ImageLayout; }
interface Itinerary { banner: BannerDetails; days: Day[]; }

// --- INITIAL DATA (Used as a fallback) ---
const initialDay: Day = { id: Date.now(), date: new Date().toISOString(), title: 'Your First Day', description: 'Describe the plan for the first day.', image: DEFAULT_IMAGES.daySummary, imageLayout: 'cover', remark: '<p>Start your amazing journey here!</p>', activeTab: 'Day Itinerary', sightseeing: [], transfers: [], meals: [] };
const initialItinerary: Itinerary = { banner: { title: 'My Awesome Trip', description: 'A brief overview of the entire journey.', image: DEFAULT_IMAGES.tripBanner, imageLayout: 'cover' }, days: [initialDay] };
...
const BannerForm: FC<{ banner: BannerDetails; onUpdate: (field: keyof BannerDetails, value: any) => void; onImageUpload: (e: ChangeEvent<HTMLInputElement>) => void; fileInputRef: React.RefObject<HTMLInputElement>; isUploading: boolean }> = memo(({ banner, onUpdate, onImageUpload, fileInputRef, isUploading }) => (
    <div className="space-y-6 animate-fade-in">
        <div><label className="text-xs font-medium text-gray-500">Main Itinerary Title</label><input type="text" placeholder="e.g., Romantic Getaway to Paris" value={banner.title} onChange={(e) => onUpdate('title', e.target.value)} className="w-full mt-1 p-2 border border-gray-300 rounded-md text-sm focus:ring-1 focus:ring-[#10A4B0] focus:border-[#10A4B0] transition" /></div>
        <div className="flex flex-col sm:flex-row gap-4">
            <div className={`flex-shrink-0 w-full sm:w-48 h-40 bg-gray-200 rounded-md relative group ${isUploading ? 'cursor-not-allowed' : 'cursor-pointer'}`} onClick={() => !isUploading && fileInputRef.current?.click()}>
                <img src={banner.image} alt="Itinerary Banner" className="w-full h-full object-cover rounded-md" />
            ...
            <textarea placeholder="Enter a brief overview for the entire trip..." value={banner.description} onChange={(e) => onUpdate('description', e.target.value)} className="w-full h-40 p-2 border border-gray-300 rounded-md text-sm resize-none focus:ring-1 focus:ring-[#10A4B0] focus:border-[#10A4B0] transition"></textarea>
        </div>
    </div>
));
...
const DayWiseItineraryPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<number | 'banner'>('banner'); // Can be 'banner' or a day's ID
  const [isPreviewOpen, setIsPreviewOpen] = useState(false);
  const [uploadingIds, setUploadingIds] = useState<Set<number>>(new Set());
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [isInitialized, setIsInitialized] = useState(false);

  // Effect for loading initial data from session storage
  useEffect(() => {
    const transformApiDataToState = (apiData: any): Itinerary => {
        if (!apiData || !apiData.itinerary || apiData.itinerary.length === 0) {
            return initialItinerary;
        }
        const startDate = new Date(apiData.startDate);
        const days = apiData.itinerary.map((apiDay: any, index: number) => {
            ...
            return { id: Date.now() + index, date: currentDate.toISOString(), title: apiDay.title || `Day ${apiDay.day}`, description: apiDay.summary || '', image: apiDay.activities[0]?.image || DEFAULT_IMAGES.daySummary, imageLayout: 'cover' as ImageLayout, remark: '', activeTab: 'Day Itinerary' as ActivityTab, sightseeing: sightseeingItems, transfers: [], meals: [], };
        });
        
        return {
            banner: {
                title: apiData.title || 'My Awesome Trip',
                description: apiData.summary || 'A brief overview of the entire journey.',
                image: days[0]?.image || DEFAULT_IMAGES.tripBanner,
                imageLayout: 'cover'
            },
            days
        };
    ...
    try {
        const editedItineraryJSON = sessionStorage.getItem('editedItinerary');
        let initialState: Itinerary;
        
        if (editedItineraryJSON) {
            const parsedData = JSON.parse(editedItineraryJSON);
            // Backward compatibility: check if it's the old array format
            if (Array.isArray(parsedData)) {
                initialState = { ...initialItinerary, days: parsedData };
            } else {
                initialState = parsedData;
            }
        } else {
            const generatedItineraryJSON = sessionStorage.getItem('generatedItinerary');
            if (generatedItineraryJSON) {
                const apiData = JSON.parse(generatedItineraryJSON);
                initialState = transformApiDataToState(apiData);
            } else {
  ...
  // Persist all changes to session storage automatically.
  useEffect(() => {
    if (isInitialized && itinerary.days.length > 0) {
        try {
            sessionStorage.setItem('editedItinerary', JSON.stringify(itinerary));
        } catch (error) {
            console.error("Failed to save itinerary to session storage:", error);
        }
    }
  }, [itinerary, isInitialized]);

  const activeDay = itinerary.days.find(d => d.id === activeTab);

  const handleUpdateBanner = useCallback((field: keyof BannerDetails, value: any) => {
    setItinerary(curr => ({ ...curr, banner: { ...curr.banner, [field]: value } }));
  }, []);

  const handleUpdateDay = useCallback((id: number, field: keyof Day, value: any) => { 
    setItinerary(curr => ({...curr, days: curr.days.map(day => day.id === id ? { ...day, [field]: value } : day)})); 
  }, []);
  ...
  const handleAddDay = useCallback(() => { 
    ...
    const newDay: Day = { id: Date.now(), date: nextDayDate.toISOString(), title: 'New Day Activity', description: 'Describe the activities for this day.', image: DEFAULT_IMAGES.daySummary, imageLayout: 'cover', remark: '', activeTab: 'Day Itinerary', sightseeing: [], transfers: [], meals: [], }; 
    setItinerary(curr => ({ ...curr, days: [...curr.days, newDay]}));
  ...
  const handleDeleteDay = useCallback((idToDelete: number) => { 
    ...
    setItinerary(curr => {
        const newDays = curr.days.filter(day => day.id !== idToDelete);
        if (activeTab === idToDelete) {
            setActiveTab(newDays[0]?.id || 'banner');
        }
        return { ...curr, days: newDays };
    });
  }, [itinerary.days, activeTab]);

  const handleImageLayoutToggle = useCallback((id: number) => {
    setItinerary(curr => ({ ...curr, days: curr.days.map(day => day.id === id ? {...day, imageLayout: day.imageLayout === 'cover' ? 'contain' : 'cover'} : day) }));
  }, []);
  ...
  const handleBannerImageUpload = (e: ChangeEvent<HTMLInputElement>) => { createUploadHandler(e, 1, (_, url) => handleUpdateBanner('image', url)); };
  ...
  const handleSave = () => {
    alert("Your latest changes have been saved successfully!");
  };
  ...
  const renderActiveForm = () => {
      if (activeTab === 'banner') {
        return <BannerForm banner={itinerary.banner} onUpdate={handleUpdateBanner} onImageUpload={handleBannerImageUpload} fileInputRef={fileInputRef} isUploading={uploadingIds.has(1)} />;
      }
  ...
  if (!isInitialized) { return <div className="flex items-center justify-center min-h-screen">Loading Itinerary...</div> }
        ...
        <main className="flex flex-col xl:flex-row gap-8 items-start">
              ...
              <div className="bg-white p-4 sm:p-6 rounded-lg border border-[#E0E0E0] shadow-sm">
                <div className="pb-3 mb-4 border-b border-gray-200">
                    <h2 className="font-semibold text-lg ">
                        {activeTab === 'banner' ? 'Trip Banner & Overview' : `Day ${itinerary.days.findIndex(d => d.id === activeTab) + 1}: ${new Date(activeDay?.date || Date.now()).toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`}
                    </h2>
                </div>
                <div className="flex items-center gap-2 pb-4 flex-wrap border-b border-gray-200">
                    <button onClick={() => setActiveTab('banner')} className={`flex items-center gap-2 px-4 py-2 text-sm rounded-md font-semibold transition-colors ${activeTab === 'banner' ? 'bg-[#10A4B0] text-white shadow-sm' : 'bg-white hover:bg-gray-100 border'}`}><Megaphone className="w-4 h-4" /> Banner</button>
                    {itinerary.days.map((day, index) => ( <button key={day.id} onClick={() => setActiveTab(day.id)} className={`px-4 py-2 text-sm rounded-md font-semibold transition-colors ${activeTab === day.id ? 'bg-[#10A4B0] text-white shadow-sm' : 'bg-white hover:bg-gray-100 border'}`}>Day {index + 1}</button> ))}
                ...
                {renderActiveForm()}
                <div className="mt-8 flex justify-center"><button onClick={handleSave} className="bg-[#10A4B0] text-white font-raleway font-medium text-sm px-10 py-2 rounded-md hover:bg-opacity-90 transition-colors">Confirm Saved Changes</button></div>
              </div>
          </div>
          <div className="w-full xl:flex-[1] order-1 xl:order-2 xl:sticky xl:top-8">
            <div className="bg-white p-4 rounded-lg border border-[#E0E0E0]">
              ...
              {activeDay && <MemoizedDayPreviewCard day={activeDay} onDelete={handleDeleteDay} onImageLayoutToggle={handleImageLayoutToggle} isDeleteDisabled={itinerary.days.length <= 1} />}
            </div>
          </div>
        </main>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-70px)]">
                <div className="mb-8">
                    <h3 className="text-2xl font-bold text-gray-800 mb-3">{itinerary.banner.title}</h3>
                    <img src={itinerary.banner.image} alt={itinerary.banner.title} className="w-full h-64 object-cover rounded-lg mb-4" />
                    <p className="text-gray-700">{itinerary.banner.description}</p>
                </div>
              {itinerary.days.map((day, index) => (
                <div key={day.id} className="mb-8 last:mb-0">
                  <h3 className="text-xl font-semibold text-gray-800 mb-3 pb-2 border-b-2 border-[#10A4B0]">Day {index + 1}: {day.title} ({new Date(day.date).toLocaleString('en-US', { month: 'short', day: 'numeric' })})</h3>
